...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Tunes lr, weight_decay, and a heavy-tail-aware u-space 'valley' conductance family "
            "(u0, left_slope, right_slope)."
        )
    )
    parser.add_argument(
        "--data-path",
    ...
    parser.add_argument(
        "--checkpoint-json",
        type=Path,
        default=Path("artifacts/criteo_fwfm/sl_optuna_u_exp_valley.checkpoint.json"),
    )
    parser.add_argument("--study-name", type=str, default="sl_optuna_u_exp_valley")
    parser.add_argument(
        "--storage-url",
        type=str,
        default="sqlite:///artifacts/criteo_fwfm/sl_optuna_u_exp_valley.sqlite3",
    )
