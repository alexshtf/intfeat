...
- Current benchmark to beat (Criteo contiguous split, Optuna best-trial val logloss): B-spline `0.47180640675774904`.
...
2. Add conductance families that can represent heavy-tail + local ROI
...
3. Sweep `c` while holding `w` fixed
...
4. Sweep `w` while holding `c` fixed
...
5. Joint sweep (small, then confirm)
...
6. Diagnostics to make "two knobs" falsifiable
